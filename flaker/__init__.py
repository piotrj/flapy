...
class Flak(object):
    ...
    def __init__(self,
        ...
        self.video = video
        self.comments = comments if not(comments) else [e if isinstance(e, Flak) else Flak(**e) for e in comments]
        self.source = source
        self.link = link
...
class Flaker(object):
    ...
    def show(self, entry_id, comments = True, html = False):
        """Return a single Flak identified by entry_id."""
        return self.flak_class(**self._request(type='show', entry_id=entry_id, comments = comments, html = html)['entries'][0])
    ...
    @flak_decoding
    def friends(self, login, since = None, html = False):
        """Get all the friends of `login`.
        """
        
        
        return self._request(type='friends', login=login, since=since, html = html)
